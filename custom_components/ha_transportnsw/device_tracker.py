...
from .helpers import extract_from_hierarchy, get_journey_data, remove_entity
...
def _is_tracker_enabled(
    tracker: str, data: dict[str, str], origin_type: str
) -> bool:
    ...
    return data[tracker] in possible_values


def _get_tripview_name(
    key, subentry_data, journey_data, device_suffix, leg_suffix
) -> str:
    ...
    try:
        ...
        transport_type = extract_from_hierarchy(
            obj=journey_data, path=f"{key_base}.type"
        )
        ...
        tripview_name = extract_from_hierarchy(obj=journey_data, path=name_path)[
            :3
        ].upper()
            ...
            for trip_index in range(0, 3, 1):
                ...
                for sensor in DEVICE_TRACKER_SENSORS:
                    if trip_index >= trips_to_create:
                        ...
                        if _is_tracker_enabled(
                            sensor.key,
                            subentry.data["device_trackers"],
                            subentry.data.get(CONF_ORIGIN_TYPE, "stop"),
                        ):
                            leg_suffix = DEVICE_TRACKER_LOOKUPS.get(sensor.key, "")
                            device_trackers.append(
                                TransportNSWDeviceTracker(
                                    coordinator,
                                    sensor,
                                    subentry,
                                    trip_index,
...
class TransportNSWDeviceTracker(CoordinatorEntity, TrackerEntity):
    ...
    def __init__(
        ...
        description: TransportNSWTrackerEntityDescription,
        ...
        if description.key == CONF_LAST_LEG_DEVICE_TRACKER:
            ...
            self._hide_if_duplicated = (
                self.subentry.data["device_trackers"][CONF_LAST_LEG_DEVICE_TRACKER]
                == "if_not_duplicated"
            )
        else:
    ...
    def latitude(self) -> float | None:
        ...
        try:
            ...
            if journey_data is not None:
                ...
                value = extract_from_hierarchy(
                    obj=journey_data,
                    path=f"{self.entity_description.state_path}.latitude",
                )
                return value

        except Exception as ex:
            _LOGGER.error(
                f"{self.subentry.title}: Error {ex} retrieving latitude for device tracker {self.entity_description.key}"
            )
    ...
    def longitude(self) -> float | None:
        ...
        try:
            ...
            if journey_data is not None:
                ...
                value = extract_from_hierarchy(
                    obj=journey_data,
                    path=f"{self.entity_description.state_path}.longitude",
                )
                return value
    ...
    def available(self) -> bool:
        ...
        try:
            ...
            if journey_data is not None:
                ...
                if self._hide_if_duplicated:
                    ...
                    entity_id = entity_reg.async_get_entity_id(
                        "device_tracker", DOMAIN, self._attr_unique_id
                    )

                    # See if we are a duplicate of CONF_FIRST_LEG_DEVICE_TRACKER
                    journey_data = self.coordinator.data[self.subentry.subentry_id][
                        self.journey_index
                    ]
                    duplicated_tracker = extract_from_hierarchy(
                        obj=journey_data,
                        path="destination_transport_detail.same_as_origin",
                        default=False,
                    )
            ...
            _LOGGER.error(
                f"{self.subentry.title}: Error {ex} setting availability for device tracker {self.entity_description.key} index {self.journey_index}"
            )
            return False
    ...
    def icon(self) -> str:
        ...
        try:
            ...
            if journey_data is not None:
                ...
                if any(
                    substring in self.entity_description.key
                    for substring in ["origin", "first"]
                ):
                    transport_type = extract_from_hierarchy(
                        obj=journey_data, path="origin_transport_detail.type"
                    )
                else:
                    transport_type = extract_from_hierarchy(
                        obj=journey_data, path="destination_transport_detail.type"
                    )
        ...
        try:
            identifiers = {
                "identifiers": {
                    (
                        DOMAIN,
                        f"{self.subentry.subentry_id}_{self.subentry.data[CONF_ORIGIN_ID]}_{self.subentry.data[CONF_DESTINATION_ID]}_{self.device_identifier}",
                    )
                },
        ...
        try:
            ...
            if journey_data is not None:
                ...
                if self.entity_description.key in [
                    ...
                    attrs["tripview_name"] = _get_tripview_name(
