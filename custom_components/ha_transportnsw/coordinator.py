...
from .helpers import get_auto_poll_interval, get_trips, within_poll_time
...
class TransportNSWRuntimeData:
    """Class to hold integration data for each config entry."""
...
async def store_api_data(api_store: Store, api_calls: int) -> int:
    ...
    if "last_reset_date" in api_data:
        ...
        last_reset_date = datetime.strptime(
            api_data["last_reset_date"], "%Y-%m-%d"
        ).date()
    ...
    api_data = {API_CALLS: api_calls, "last_reset_date": str(last_reset_date)}
...
class TransportNSWCoordinator(DataUpdateCoordinator):
    ...
    def __init__(
        self, hass: HomeAssistant, config_entry: TransportNSWConfigEntry
    ) -> None:
    ...
    async def _async_update_data(self):
        ...
        if self.daily_api_calls == 0:
            ...
            try:
                ...
                if api_data is None or api_data.get(API_CALLS, None) is None:
                    ...
                    prorated_api_calls = int(
                        (1 - (minutes_remaining / 1440)) * API_DAILY_LIMIT
                    )
        ...
        for subentry in self.config_entry.subentries.values():
            ...
            if subentry.subentry_type == SUBENTRY_TYPE_JOURNEY:
                ...
                if within_poll_time(subentry)[0]:
                    ...
                    if (
                        CONF_ORIGIN_TYPE in subentry.data
                        and subentry.data[CONF_ORIGIN_TYPE] == "device_tracker"
                    ):
                        try:
                            ...
                            if self.config_entry.options.get(
                                CONF_REQUEST_LOCATION_UPDATE, False
                            ):
                                _LOGGER.debug(
                                    f"Requesting location update from {subentry.data[CONF_ORIGIN_ID]}"
                                )
                            ...
                            origin_coordinates = find_coordinates(
                                self.hass, subentry.data[CONF_ORIGIN_ID]
                            )
                            ...
                            raise UpdateFailed(
                                f"Error {ex} retrieving coordinates from {subentry.data[CONF_ORIGIN_ID]}"
                            ) from ex
                    ...
                    try:
                        ...
                        origin_transport_list = [
                            int(transport_type)
                            for transport_type in subentry.data[
                                CONF_ORIGIN_TRANSPORT_TYPE
                            ]
                        ]
                        ...
                        _LOGGER.debug(
                            f"Calling get_trips: origin = {origin}, destination_id = {subentry.data[CONF_DESTINATION_ID]}, trip_wait_time = {subentry.data[CONF_TRIP_WAIT_TIME]}, journeys_to_return = {subentry.data[CONF_TRIPS_TO_CREATE]}, origin_transport_type = {subentry.data[CONF_ORIGIN_TRANSPORT_TYPE]}, destination_transport_type = {subentry.data[CONF_DESTINATION_TRANSPORT_TYPE]}, route_filter = {subentry.data[CONF_ROUTE_FILTER]}, run_filter = {subentry.data[CONF_RUN_FILTER]}, include_realtime_location = True, max_changes = {subentry.data[CONF_MAX_CHANGES]}"
                        )
                        ...
                        if (
                            journey_data is not None
                            and "journeys_with_data" in journey_data
                            and journey_data["journeys_with_data"] > 0
                        ):
                            if (
                                journey_data["journeys_to_return"]
                                > journey_data["journeys_with_data"]
                            ):
                                ...
                                if subentry.data[CONF_ORIGIN_TRANSPORT_TYPE] == ["11"]:
                                    ...
                                    _LOGGER.warning(
                                        f"{subentry.title}: {journey_data['journeys_to_return']} journeys were requested but only got {journey_data['journeys_with_data']}, most likely because school bus journeys only run on weekdays."
                                    )
                                else:
                                    _LOGGER.warning(
                                        f"{subentry.title}: {journey_data['journeys_to_return']} journeys were requested but only got {journey_data['journeys_with_data']} - consider relaxing the journey restrictions."
                                    )

                            if "journeys" in journey_data:
                                returned_data[subentry.subentry_id] = journey_data[
                                    "journeys"
                                ]
                            ...
                            if subentry.data[CONF_ORIGIN_TRANSPORT_TYPE] == ["1"]:
                                _LOGGER.warning(
                                    f"{subentry.title}: no journeys returned for this train-only journey - there may be a bus replacement service active at the moment."
                                )
                            else:
                                _LOGGER.warning(
                                    f"{subentry.title}: no journeys returned - consider relaxing the journey restrictions."
                                )
                        ...
                        raise UpdateFailed(
                            f"Error communicating with API for entry {subentry.title}: {type(ex).__name__}"
                        ) from ex
        ...
        if len(self._rolling_average_api_calls_list) < AVERAGE_API_CALLS_WINDOW:
            ...
            self._rolling_average_api_calls_list = [
                integration_api_count,
                *self._rolling_average_api_calls_list[1:],
            ]

        # Round the result, but don't use Banker's Rounding
        average = sum(self._rolling_average_api_calls_list) / len(
            self._rolling_average_api_calls_list
        )
        ...
        if (
            ...
            daily_api_allocation = (
                self.config_entry.options.get(CONF_API_PERCENT, DEFAULT_API_PERCENT)
                / 100
            )
            new_update_interval = get_auto_poll_interval(
                self, daily_api_allocation, latest_end_time
            )
        ...
        self.daily_api_calls = await store_api_data(
            self.config_entry.runtime_data.api_store, self.daily_api_calls
        )
